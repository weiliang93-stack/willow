...
-- shared/supabase-schema.sql, to support sheet-training-sync.
--
-- Tracks which training-app log entries (by their `id`) have already been
-- pushed to the Google Sheet, so re-running the sync only appends rows for
-- sets logged since the last run instead of re-appending everything and
...
create table if not exists public.training_sheet_sync_state (
  id smallint primary key,
  synced_ids jsonb not null default '[]'::jsonb,
  updated_at timestamptz not null default now()
);

insert into public.training_sheet_sync_state (id, synced_ids)
values (1, '[]'::jsonb)
on conflict (id) do nothing;

alter table public.training_sheet_sync_state enable row level security;

-- No policies added on purpose: this table is only ever read/written by
-- the sheet-training-sync Edge Function using the service_role key, which
